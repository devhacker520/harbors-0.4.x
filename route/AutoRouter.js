...
harbors.AutoRouter = harbors.Router.extend({
    ...
    _gzip: null,
    _gzipList: ['.js', '.css', '.html'],

    _deflate: null,
    _deflateList: ['.js', '.css', '.html'],
    ...
    _showFile: function(file, req, res){
        ...
        if(this._modifiedCache){
            ...
            res.setHeader("Content-Type", this._mime[extname] || this._mime[".html"]);

            if(lastModified == req.headers[ifModifiedSince]){

                this._returnData(304, null, null, res, req, extname);

            }else{

                this._returnData(200, {
                    "Last-Modified": lastModified
                }, fs.readFileSync(file), res, req, extname);
            }


        }else{

            this._returnData(200, null, fs.readFileSync(file), res, req, extname);
        }


    },

    _accessGzip: function(extname){
        for(var i=0; i< this._gzipList.length; i++){
            if(this._gzipList[i] === extname){
                return true;
            }
        }
        return false;
    ...
    _accessDeflate: function(extname){
        for(var i=0; i< this._deflateList.length; i++){
            if(this._deflateList[i] === extname){
                return true;
            }
        }
        return false;
    ...
    _returnData: function(statCode, headers, buffer, res, req, extname){
        ...
        if(this._gzip && acceptEncoding && acceptEncoding.indexOf('gzip') > -1 && this._accessGzip(extname)){
        ...
        if(this._deflate && acceptEncoding && acceptEncoding.indexOf('deflate') > -1 && this._accessDeflate(extname)){
